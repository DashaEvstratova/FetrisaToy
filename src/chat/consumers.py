...
from main_store.models import Items
...
class MainConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        is_note_exist = await Items.objects.filter(id=room_id).aexists()
        if not is_note_exist:
            await self.close()
            return
        await self.channel_layer.group_add(room_id, self.channel_name)
        await self.accept()
        items = await Items.objects.filter(id=room_id).aget()
        await self.send(json.dumps({"message": items.name}))
